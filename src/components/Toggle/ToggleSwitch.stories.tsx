...
const ToggleSwitchStory = ({ id = `toggleSwitch`, name = id, debug, ...props}: Partial<ToggleSwitchProps> & { debug?: boolean }) => {
  ...
  return (
    ...
    />
    {debug && (
<p css={(theme: Theme) => ({ fontFamily: theme.fonts.monospace, padding: theme.space[3], background: theme.colors.grey[10], fontSize: theme.fontSizes[1] })}>
...
export const Tones = () => TOGGLE_TONES.map((tone) => (
  <div key={tone}>
    <p id={`toggleSwitchLabel--${tone}`} css={(theme: Theme) => ({ marginBottom: theme.space[2], fontWeight: theme.fontWeights.bold })}>{tone}:</p>
  <ToggleSwitchStory
    id={`tone--${tone}`}
    tone={tone}
    value="on"
